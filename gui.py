...
class SudokuSolver():
    ...
    # Setting the empty cells to 0 (i.e. the cells you have not filled in)
    def setZero(self):
        for i in range(9):
            for j in range(9):
    ...
    # Checking the row, column and subgrid(3x3 square) to see if number can be placed in cell          
    ...
    # Finding empty cells, defined as cells filled with a zero
    def emptyCell(self):
        for row in range(9):
...
class Interface():
    def __init__(self, window):
        ...
        for row in range(9):
            for col in range(9):
                ...
                # Making each cell of grid a entry box and store each user entry into the filledBoard 2D list
    ...
    # Calling Sudoku solver class (called by solve button)
    def Solve(self):
        SudokuSolver()
